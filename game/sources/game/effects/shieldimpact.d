module game.effects.shieldimpact;
...
class ShieldImpact : GameObject, ISerializeable {
private:
	struct Params {
		struct SpriteParams {
	...
	static class Proxy : RenderProxyGameObject!(ObjectInfoOrientedSprite) {
		private: 
      int i;
      ShieldImpact m_outer;

    public:
      this(ShieldImpact outer)
      {
        m_outer = outer;
      }
		
		override void initInfo(ref ObjectInfoOrientedSprite info){
			info.color = m_outer.m_Params.color * (m_outer.m_TimeToLive / m_outer.m_Params.timeToLive.value);
...
public:
	...
	static void loadXml(){
		m_Loaded = true;
		FromXmlFile(m_Params,_T("gfx/xml/shieldimpact.xml"));
		if(!g_Env.isServer){
