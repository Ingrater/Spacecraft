module game.effects.smallexplosion;
...
class SmallExplosion : GameObject, ISerializeable {
private:
	struct Params {
		struct SpriteParams {
    ...
    private SmallExplosion m_outer;

    this(SmallExplosion outer)
    {
      m_outer = outer;
    }
		...
		override void initInfo(ref ObjectInfoSprite info){
			...
			if(spriteId >= int_cast!uint(m_outer.m_Sprites.length))
