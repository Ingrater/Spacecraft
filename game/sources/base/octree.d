...
private struct OctreeObjectInfo {
  Node node;
...
private enum ExtendDirection : ubyte {
  NEGATIVE,
  POSITIVE
}
...
public:
  ...
  */
  this(Octree outer, Position center, float size){
    m_outer = outer;
    m_Center = center;
    m_RealSize = size;
  ...
  */
  this(Octree outer, Node child, ExtendDirection extendDirection){
    m_outer = outer;
    m_Objects = New!(typeof(m_Objects))(StdAllocator.globalInstance);
    m_HasChilds = true;
    ...
    if(extendDirection == ExtendDirection.NEGATIVE){
      m_Childs[0] = new Node(outer, center + vec3(shift,shift,shift),child.m_RealSize);
      m_Childs[1] = new Node(outer, center + vec3( 0.0f,shift,shift),child.m_RealSize);
      m_Childs[2] = new Node(outer, center + vec3(shift, 0.0f,shift),child.m_RealSize);
      m_Childs[3] = new Node(outer, center + vec3( 0.0f, 0.0f,shift),child.m_RealSize);
      m_Childs[4] = new Node(outer, center + vec3(shift,shift, 0.0f),child.m_RealSize);
      m_Childs[5] = new Node(outer, center + vec3( 0.0f,shift, 0.0f),child.m_RealSize);
      m_Childs[6] = new Node(outer, center + vec3(shift, 0.0f, 0.0f),child.m_RealSize);
      m_Childs[7] = child;
    }
    else {
      m_Childs[0] = child;
      m_Childs[1] = new Node(outer, center + vec3(shift, 0.0f, 0.0f),child.m_RealSize);
      m_Childs[2] = new Node(outer, center + vec3( 0.0f,shift, 0.0f),child.m_RealSize);
      m_Childs[3] = new Node(outer, center + vec3(shift,shift, 0.0f),child.m_RealSize);
      m_Childs[4] = new Node(outer, center + vec3( 0.0f, 0.0f,shift),child.m_RealSize);
      m_Childs[5] = new Node(outer, center + vec3(shift, 0.0f,shift),child.m_RealSize);
      m_Childs[6] = new Node(outer, center + vec3( 0.0f,shift,shift),child.m_RealSize);
      m_Childs[7] = new Node(outer, center + vec3(shift,shift,shift),child.m_RealSize);
    }
  }
  ...
  body {
    ...
    m_Childs[0] = New!Node(m_outer, m_Center + vec3(-shift,-shift,-shift),newsize);
    m_Childs[1] = New!Node(m_outer, m_Center + vec3( shift,-shift,-shift),newsize);
    m_Childs[2] = New!Node(m_outer, m_Center + vec3(-shift, shift,-shift),newsize);
    m_Childs[3] = New!Node(m_outer, m_Center + vec3( shift, shift,-shift),newsize);
    m_Childs[4] = New!Node(m_outer, m_Center + vec3(-shift,-shift, shift),newsize);
    m_Childs[5] = New!Node(m_outer, m_Center + vec3( shift,-shift, shift),newsize);
    m_Childs[6] = New!Node(m_outer, m_Center + vec3(-shift, shift, shift),newsize);
    m_Childs[7] = New!Node(m_outer, m_Center + vec3( shift, shift, shift),newsize);
    ...
    while(!r.empty){
      ...
      foreach(node;m_Childs){
        if(box in node.m_BoundingBox){
          ...
          m_outer.changeObjectLocation(node,node.m_Objects.back());
          moved = true;
          break;
        }
      }
    ...
    foreach(child;m_Childs){
      if(!child.m_HasChilds && child.m_Objects.size() >= 8 && child.m_RealSize > m_outer.m_MinSize){
        child.subdivide();
      }
    }
      ...
      if(numObjects < 8 && doOptimization){
        foreach(child;m_Childs){
          ...
          for(auto r = child.m_Objects[];!r.empty();){
            ...
            m_outer.changeObjectLocation(this,m_Objects.back());
          }
          assert(child.m_Objects.empty());
          debug {
            //Check if there is still a refrence to this node in any of the OctreeObjectInfo objects
            foreach(ref OctreeObjectInfo info; m_outer.m_ObjectInNode.values)
            {
  ...
  */
  bool insert(IGameObject obj){
    ...
    if(obj.boundingBox in m_BoundingBox){
      ...
      m_outer.changeObjectLocation(this,m_Objects.back());
      if(!m_HasChilds && m_Objects.size() >= 8 && m_RealSize > m_outer.m_MinSize){
        subdivide();
      }
      return true;
    }
...
private:		
	Node m_Root;	
  Hashmap!(IGameObject, OctreeObjectInfo, ReferenceHashPolicy) m_ObjectInNode;
	...
	void changeObjectLocation(Node node, DoubleLinkedList!(IGameObject).Range r){
		m_ObjectInNode[r.front()] = OctreeObjectInfo(node,r);
	}
...
public:
	...
	this(float startSize, float minSize){
		m_Root = new Node(this, Position(vec3(0,0,0)),startSize);
		m_MinSize = minSize;
  ...
  {
    m_ObjectInNode.removeWhere((ref IGameObject obj, ref OctreeObjectInfo info){
	...
	void insert(IGameObject obj){
		...
		while( !(obj.boundingBox in m_Root.m_BoundingBox) ){
			...
			m_Root = New!Node(this, m_Root,m_ExtendDirection);
