module game.effects.particle_oo_v6;
...
final class ParticleSystem : RenderProxyRenderable!(ObjectInfoSprite), IGameObject
{
  ...
  this(ParticleEmitter emitter)
  {
    ...
    m_particleAlives = new Vector!bool();
  ...
  Particle* SpawnParticle()
  {
    ...
    else
    {
      ...
      auto aliveRoot = m_particleAlives.toArray().ptr;
      ...
      if(aliveRoot != m_particleAlives.toArray().ptr)
      {
        ReassignPointers!("m_particleAlives","m_alive")();
      }
      ...
      result.m_alive = &m_particleAlives[index];
    }
    *result.m_alive = true;
    return result;
  }
  ...
  auto GetParticleAliveIterator()
  {
    return m_particleAlives.toArray();
  }
  ...
  {
    base.logger.info("%s = %s",name,timeDiff);
  }
  ...
  override void update(float deltaTime)
  {
    ...
    {
      auto profile = base.profiler.Profile("particle emit", &log);
      m_emitter.Emit(this, deltaTime);
    }

    {
      auto profile = base.profiler.Profile("particle update", &log);
      size_t end = 1000;
      for(; end <= m_particles.length; end += 1000)
      {
        foreach(mod;GetModifier())
        {
          mod.Apply(deltaTime, this, end - 1000, end);
        }
      }
      if(end < m_particles.length)
      {
        foreach(mod;GetModifier())
        {
          mod.Apply(deltaTime, this, end, m_particles.length);
        }
      }
    }

    {
      auto profile = base.profiler.Profile("particle bounding box", &log);
...
{
  void Apply(float deltaTime, ParticleSystem particleSystem, size_t start, size_t size);
...
{
  void Apply(float deltaTime, ParticleSystem particleSystem, size_t start, size_t end)
  {
    auto profile = base.profiler.Profile("standard modifier", &particleSystem.log);
    foreach(ref particle;particleSystem.GetParticles()[start..end])
    {
      if(!*particle.m_alive)
        continue;
...
final class ParticleModifierWind : ParticleModifier
{
  ...
  void Apply(float deltaTime, ParticleSystem particleSystem, size_t start, size_t end)
  {
    auto profile = base.profiler.Profile("wind modifier", &particleSystem.log);
    auto velocitys = particleSystem.GetParticleVelocityIterator();
    auto alives = particleSystem.GetParticleAliveIterator();

    auto curVelocity = velocitys.ptr + start;
    auto curAlive = alives.ptr + start;
    auto endVelocity = velocitys.ptr + end;
    auto endAlive = alives.ptr + end;

    for(; curVelocity < endVelocity && curAlive < endAlive; ++curVelocity,++curAlive)
    {
      if(*curAlive == true)
        *curVelocity += m_windDirection * deltaTime;
    }
  }
...
class ParticleModifierColorGradient : ParticleModifier
{
  ...
  void Apply(float deltaTime, ParticleSystem particleSystem, size_t start, size_t end)
  {
    auto profile = base.profiler.Profile("gradient modifier", &particleSystem.log);
    auto color = particleSystem.GetParticleColorIterator();
    auto ageInfo = particleSystem.GetParticleAgeInfoIterator();
    auto alives = particleSystem.GetParticleAliveIterator();
    
    auto curColor = color.ptr + start;
    auto curAgeInfo = ageInfo.ptr + start;
    auto curAlive = alives.ptr + start;
    auto endColor = color.ptr + end;
    auto endAgeInfo = ageInfo.ptr + end;
    auto endAlive = alives.ptr + end;
    for(;curColor < endColor && curAgeInfo < endAgeInfo && curAlive < endAlive; ++curColor,++curAgeInfo,++curAlive)
    {
      if(*curAlive == true)
      {
